...
import { Plus, FileText, User, Warehouse } from "lucide-react";
...
export default function SalesPage() {
  ...
  const fetchSales = async () => {
    ...
    const { data } = await supabase
      .from("sales")
      .select(`id, invoice_number, total_amount, created_at, customers ( name ), warehouses ( name )`)
      .is("deleted_at", null)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-text-primary">المبيعات</h1>
          <p className="text-text-secondary mt-1">فواتير المبيعات للعملاء</p>
        </div>
      ...
      <div className="bg-card rounded-2xl shadow-soft border border-border overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-text-secondary flex flex-col items-center gap-2">
            <FileText className="w-12 h-12 text-border" /> لا توجد فواتير مبيعات
          </div>
        ) : (
                ...
                {sales.map((s) => (
                  <tr key={s.id} className="hover:bg-background transition-colors">
                    <td className="py-4 px-6 font-medium text-text-primary flex items-center gap-2">
                      ...
                      {s.invoice_number || s.id.substring(0, 8)}
                    </td>
                    <td className="py-4 px-6 text-text-secondary flex items-center gap-1"><User className="w-4 h-4" /> {s.customers?.name || "نقدي"}</td>
                    <td className="py-4 px-6 text-text-secondary flex items-center gap-1"><Warehouse className="w-4 h-4" /> {s.warehouses?.name}</td>
                    <td className="py-4 px-6 font-bold text-primary-green">{s.total_amount.toLocaleString()}</td>
                    <td className="py-4 px-6 text-text-secondary text-sm">{new Date(s.created_at).toLocaleDateString('ar-EG')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <SaleForm isOpen={isFormOpen} onClose={handleClose} />
    </div>
  );
}
